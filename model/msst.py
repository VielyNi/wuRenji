...
class MSSTNet(nn.Module):
    ...
    def __init__(self, num_class=1000, dropout=0.8,inchannel=3):
        ...
        self.avgpool2 = nn.Sequential(
            ...
            nn.ReLU(inplace))
        inc = inchannel
        self.m1 = msstMoudel(inchannel=inc, outchannel1=32, outchannel3=48, outchannel5=48, outchannel7=48,
...
class Model(nn.Module):
    def __init__(self, num_class=60, dropout=0.8,inchannel=3):
        super(Model, self).__init__()
        self.dropout = dropout
        self.num_class = num_class
        self.inc = inchannel
        self.model = MSSTNet(num_class=self.num_class, dropout=self.dropout,inchannel=self.inc)
    ##########################原处理方式#############################################
    # def forward(self, input):
    #     # print(self.model)
...
if __name__=='__main__':
    ...
    model = Model(num_class=155, dropout=0.8)
    input_sk = torch.rand([4, 3, 300, 17, 2])
    out = model(input_sk)
    print(out[1].size())
    print(out[0].size())
