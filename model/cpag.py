...
class CPAG_FPH(nn.Module):
    def __init__(self, in_c, h_in, h_out, w):
        super().__init__()
        self.cpa_p = CPAG(in_c, h_in, h_out, w)
        self.cpa_n = CPAG(in_c, h_in, h_out, w)
        # self.fph = FPH(1, fin, fout)
    
    def forward(self, x):
        x = rearrange(x, '(n m) c s w -> n c s w m', m = 2)
        attp = self.cpa_p(x)
        x_attp = x * attp
        attn = self.cpa_n(x)
        x_attn = x * attn
        x = rearrange(x, ' n c s w m->(n m) c s w ')
        x_attp = rearrange(x_attp, ' n c s w m->(n m) c s w ')
        x_attn = rearrange(x_attn, ' n c s w m->(n m) c s w ')
        return x, x_attp, x_attn
        # print(x)
        # f = self.fph(x_att)
        # print(f)
