...
class Processor():
    ...
    def load_model(self):
        ...
        self.graphContrast = InfoNCEGraph(in_channels=3*17*17, out_channels=256, class_num=self.arg.model_args["num_class"], \
            mem_size=mem_size, label_all=label_all, T=0.8).cuda(output_device)
    ...
    def load_optimizer(self):
        if self.arg.optimizer == 'SGD':
            ...
            self.optimizer_graph = optim.SGD(
                self.graphContrast.parameters(),
                lr=self.arg.base_lr,
                momentum=0.9,
                nesterov=self.arg.nesterov,
                weight_decay=self.arg.weight_decay)
            ...
            self.optimizer = optim.Adam(
                ...
                lr=self.arg.base_lr,
                weight_decay=self.arg.weight_decay)
            self.optimizer_graph = optim.Adam(
                self.graphContrast.parameters(),
                lr=self.arg.base_lr,
                weight_decay=self.arg.weight_decay)
        else:
        ...
        lr_scheduler_pre_graph = optim.lr_scheduler.MultiStepLR(
            self.optimizer_graph, milestones=self.arg.step, gamma=0.1)

        self.lr_scheduler = GradualWarmupScheduler(self.optimizer, total_epoch=self.arg.warm_up_epoch,
                                                   after_scheduler=lr_scheduler_pre)
        self.lr_scheduler_graph = GradualWarmupScheduler(self.optimizer_graph, total_epoch=self.arg.warm_up_epoch,
                                                   after_scheduler=lr_scheduler_pre_graph)
    ...
    def adjust_learning_rate(self, epoch):
        if self.arg.optimizer == 'SGD' or self.arg.optimizer == 'Adam':
            ...
            raise ValueError()
    def adjust_learning_rate_graph(self, epoch):
        if self.arg.optimizer == 'SGD' or self.arg.optimizer == 'Adam':
            if epoch < self.arg.warm_up_epoch:
                lr = self.arg.base_lr * (epoch + 1) / self.arg.warm_up_epoch
            else:
                lr = self.arg.base_lr * (
                        0.1 ** np.sum(epoch >= np.array(self.arg.step)))
            for param_group in self.optimizer_graph.param_groups:
                param_group['lr'] = lr
            return lr
        else:
            raise ValueError()
    ...
    def train(self, epoch, save_model=False):
        ...
        self.adjust_learning_rate_graph(epoch)
