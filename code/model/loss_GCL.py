...
class InfoNCEGraph(nn.Module):
    def __init__(self, in_channels=128, out_channels=256, mem_size=512, positive_num=128, negative_num=512, T=0.8, class_num=155, label_all=[]):
    ...
    def forward(self, f, label, input_index):
        # f: n c label: n
        label_=torch.zeros((label.size(0),1))
        for i in range (label.size(0)):
            label_[i]=torch.argmax(label[i,:],dim=-1)
        ...
        bank_label = self.label_all.to(label_.device) # mem_size
        positive_mask = (label_.view(n, 1) == bank_label.view(1, -1)).view(n, self.mem_size).cuda() # n mem_size
